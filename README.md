...
All required python packages can be installed with the crfnet pip package. Run the following command in the repository folder (crfnet).
...
Dockerfile      | Dockerfile to setup the environment. Start with  `nvidia-docker run -it --net=host -e NVIDIA_VISIBLE_DEVICES=0 -e USERID=$UID -v ($HOME)/data:/data --name crfnet_gpu0 crfnet`.
...
## Usage Docker Image
The use of the docker image is optional, but recommended to run the repository in an independent environment.
...
2. Start the docker using GPU 0 and the container name `crfnet_gpu0` via
    ...
    5. Start container: `docker run -it --net=host --gpus '"device=0"' -e USERID=$UID -v $HOST_DATA_DIR:/root/data -v $HOST_LOG_DIR:/crfnet/tb_logs -v $HOST_MODEL_DIR:/crfnet/saved_models -e DISPLAY=$DISPLAY --name crfnet_gpu0 crfnet`
 

3. The repository is located at `/crfnet` inside the docker and already installed when building the docker. You are automatically connected to the docker. Additionally you can connect to it via

    `docker attach crfnet_gpu0`
